  ...
  <div class="my-box roomPage" v-loading="loading">
    ...
    <el-row class="notice" v-if="this.noticeList != false">
      ...
      <div class="notice_list">
        <div class="animate">
          <span v-for="(item, index) in noticeList">{{item.buildingName + item.floorName + item.roomNumber}}:{{ item.status }}</span>
        </div>
      </div>
      ...
      <el-card shadow="always">
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item
            ...
            :name=item.id>
            <template slot="title">
              <div class="building" style="display: inline-flex;flex-direction: row;align-items: center;">
                <img src="../../../static/images/icon/buildingName.png" style="margin-right: 5px;">
                <span>{{item.buildingName}}</span>
              </div>
              <div class="floor" style="display: inline-flex;flex-direction: row;align-items: center;">
                <img src="../../../static/images/icon/floor.png" style="margin-right: 5px;margin-left: 5px;">
                <span>{{item.floorName}}</span>
              </div>
            </template>
            <el-card shadow="hover" @click.native="openDialog(item1)" v-for="(item1, index) in item.roomStatusRealtimeList" :class="[ item1.rent == '1' ? 'rentBackground' : '' ]">
              <div class="room_item">
                <span class="room_num">
                  <span>{{item1.roomNumber}}</span>
                  <img src='../../../static/images/icon/i_sos.png' v-if="item1.sos == '1' && item1.basicstatus =='on'">
                </span>
          ...
          <div class="equipment_info">
            <div class="equipment_item" v-for="(item, index) in getRoomDeviceArr.tc">
              ...
              <div class="air_info" v-if="item.deviceStatus == '开'">
                <el-input-number v-model="item.settingValue" :min="16" :max="32" controls-position="right" @change="handleChangeTemp($event, item)" size="large" style="font-size: 20px;"></el-input-number>
                <div>
                  <span>模式：</span>
                  <span>
                    <el-radio-group v-model="item.valueStatus" size="mini" @change="changeMode($event, item)">
                ...
                <div>
                  <span>风速：</span>
                  <span>
                    <el-radio-group v-model="item.speedStatus" size="mini" @change="changeAirSpeed($event, item)">
                      <el-radio-button label="关闭"></el-radio-button>
              ...
              <div style="margin-top: 3px;">
                <el-radio-group v-model="item.deviceStatus" size="small" @change="changeAir($event, item)">
            ...
            <div class="equipment_item" v-for="(item, index) in getRoomDeviceArr.light">
              ...
              <img src="../../../static/images/dialog/lamp_open.png" v-if="item.deviceStatus == '开'">
              <img src="../../../static/images/dialog/lamp_close.png" v-else>
              <div>
                <el-radio-group v-model="item.deviceStatus" size="small" @change="changeLight($event, item)">
                  <el-radio-button label="开"></el-radio-button>
                  <el-radio-button label="关"></el-radio-button>
                </el-radio-group>
              </div>
            ...
            <span v-if="getRoomDeviceArr.tc == false && getRoomDeviceArr.fan == false && getRoomDeviceArr.light == false" class="noHaveData">暂无数据</span>
          </div>
        </div>
      </div>
    ...
    <el-dialog title="消息" :visible.sync="openNoticeDialogVisible" width="30%">
      <div style="max-height: 400px;overflow-y: scroll;">
        <div v-for="(item, index) in noticeList">
          {{item.buildingName + item.floorName + item.roomNumber}}:{{ item.status }}
        </div>
      </div>
    </el-dialog>
  </div>
